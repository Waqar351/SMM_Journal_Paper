...
def ensemble_binary2multi_OVR(X_test, counter, models_bin, list_scorer_bin, list_scores_bin, n_classes):
    ...
    p_hat = np.zeros((len(models_bin), n_classes))
    for i in range(0, len(models_bin)):
        dist_predicted = []
        for ci in range(n_classes):
            clf = list_scorer_bin[i][ci]
            scores = deepcopy(list_scores_bin[i][ci])                     
            #pdb.set_trace()
            ...
            pred_pos_prop = apply_quantifier(qntMethod = quantifier, p_score = pos_scores, n_score = neg_scores, test_score = te_scores, 
                                    TprFpr = tprfpr, thr = np.round(np.median(tprfpr['threshold']),1), measure = measure, calib_clf = clf, 
        ...
        p_hat[i] = dist_predicted 
    
    dist_predicted = np.median(p_hat, axis = 0) 
    pdb.set_trace()
    return (dist_predicted/np.sum(dist_predicted))
