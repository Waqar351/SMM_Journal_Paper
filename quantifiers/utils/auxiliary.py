...
def getScores(dt, label, folds, clf, proba=True):
    ...
    Y_cts = np.unique(label, return_counts=True)
    ...
    if nfolds > 1:
        ...
        clf.fit(dt, label)  
        results.extend(clf.predict_proba(dt)[:,1])
        class_labl.extend(label)
